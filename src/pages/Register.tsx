...
const Register: React.FC = () => {
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    .then(response => {
      ...
      setModalMessage(response.data.message || 'User registered successfully. You will be redirected to login.');
      setIsModalOpen(true);
    })
    .catch(error => {
      // On error (any status other than 2xx)
      if (error.response) {
  ...
  const closeModal = () => {
    ...
    if (modalTitle === 'Registration Successful') {
      navigate('/'); // Redirect to login only on success
    }
  };
